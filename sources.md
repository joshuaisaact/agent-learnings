...
## δ-mem — Lei, Zhang, et al. (May 2026)

[arxiv.org/abs/2605.12357](https://arxiv.org/abs/2605.12357)

Research paper proposing a compact online memory state coupled directly to attention, as an alternative to textual memory (RAG, summarization, context replay). Tracked here as a research direction — see [research-directions.md](research-directions.md) for the framing of why it matters; this entry covers what I took from reading the paper itself.

**What I took:** Three ideas that change how I think about memory for agents.

First, the core mechanism is genuinely different from prior approaches. δ-mem maintains an 8×8 matrix alongside a frozen Transformer, updated by delta-rule learning (online SGD on a key-value association loss) as tokens are processed. At inference, the current input queries the matrix and the result is projected into low-rank corrections to the attention query and output — not retrieved text, not retrieved embeddings, but direct steering signals injected into the forward pass. This is fundamentally different from RAG (retrieves text, appends to context), from LoRA (static parametric update), and from external KV-memory approaches like Memorizing Transformers (which still retrieve through a separate pathway). The memory is dynamic *and* directly couples into the backbone's own computation.

Second, the no-context experiment is the most striking result. With explicit context *removed* — the model never sees the source content in the question turn — δ-mem still scores 6.48% EM on HotpotQA vs 0.08% for the no-context baseline (an 80x relative improvement on a small absolute number, but still). On LoCoMo, no-context average rises from 3.49% to 8.05% with clear gains across multi-hop, temporal, open-domain, and single-hop questions. The 8×8 matrix is encoding enough signal from prior turns to recover meaningful answers without any text replay. This falsifies the assumption baked into most agent memory systems that effective long-term memory requires text-level recall.

Third, textual memory baselines actively *hurt* the unaugmented backbone. On Qwen3-4B-Instruct, the base model scores 46.79% average. BM25 RAG drops it to 44.56%. MemoryBank: 43.88%. LLMLingua-2: 42.96%. MLP Memory: 22.85%. δ-mem improves it to 51.66%. The textual approaches degrade general capability because they fill the context window with retrieved noise that crowds out useful tokens. δ-mem doesn't because it never touches the input. This is a direct empirical case against the default agent-memory pattern of "shove retrieved history into the prompt."

Also worth noting: three writing strategies (token-level, message-segment averaged, multi-state with parallel sub-matrices), and the best one depends on backbone size — segment-level smooths token noise for large backbones like Qwen3-8B, while multi-state reduces interference for small backbones like SmolLM3-3B that can't easily separate competing memory categories in a single state. Insertion-depth ablation finds middle layers are the sweet spot for partial-layer injection, and the "all layers" configuration is best overall.

**Key insight:** Almost every memory pattern in this repo — CLAUDE.md, progress files, get-bearings ritual, session log as external context object, compaction — is scaffolding that exists because current models lack effective non-textual memory. If something like δ-mem ships in production models, much of that scaffolding becomes legacy infrastructure. The progress file is downstream of a model-level deficiency, not an inherent property of long-running work. Worth knowing what you'd stop doing when this changes.

Practical caveats before getting excited: only trained and evaluated up to ~8K-token write budgets, so real long-horizon agent traces (100K+ tokens) are untested. Decoding is slower than vanilla because each step reads and writes the state, though GPU memory footprint stays flat even at 32K prompt length. Trainable params are tiny (4.87M for SSW/TSW = 0.12% of backbone; 19.47M for MSW = 0.48%), but you still need to train them — there's no off-the-shelf δ-mem to drop into Claude or GPT.
