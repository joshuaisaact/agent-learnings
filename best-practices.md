...
- **Map, not manual** — keep instruction files short (~100 lines) and use them as a table of contents pointing to deeper sources of truth. OpenAI tried one big AGENTS.md and it failed: context is scarce so a giant file crowds out the actual task, too much guidance becomes non-guidance, monolithic files rot instantly and agents can't tell what's still true, and a single blob is hard to verify mechanically for freshness or coverage. A structured docs/ directory with indexed, cross-linked artifacts works better. Now empirically validated: the JAWs 2026 study (124 PRs, Codex gpt-5.2-codex) found AGENTS.md reduces median runtime by 28.6% and output tokens by 16.6% — but only when hand-written and concise. LLM-generated AGENTS.md files *hurt* performance by 2-3% because they're redundant to existing docs (READMEs, contributing guides) that the agent can already read. And bloated files are actively harmful: a 1,247-line CLAUDE.md caused 14 turns with 2 bugs; trimmed to 47 essential lines, the same task completed in 8 turns with zero bugs. The mechanism: without AGENTS.md, the agent explores (reads directories, infers build systems, guesses test commands); with a concise one, it skips straight to working. But a bloated one dilutes attention — the agent can't distinguish what matters from what's filler.
...
## Autonomy grows with trust, not capability

Anthropic analyzed 998K tool calls from their API and found that autonomy is a trust problem, not a capability problem. The 99.9th percentile turn duration nearly doubled in 3 months (under 25 minutes → over 45 minutes), but this growth is smooth across model releases — it doesn't jump with new model launches. If it were purely about capability, you'd see sharp jumps. Instead it's users building trust over time and applying agents to more ambitious tasks.

The trust curve is non-obvious: experienced users auto-approve more (20% of sessions for new users → 40%+ by 750 sessions) but also *interrupt more often*. More autonomy and more oversight coexist — they're not opposites. Users learn when to trust and when to check.

Deployment concentration is stark: software engineering is 49.7% of all tool calls. The rest is a long tail — back-office automation (9.1%), marketing (4.4%), sales/CRM (4.3%), finance (4.0%), data analysis (3.5%). Agent autonomy is a software engineering phenomenon with a deployment overhang everywhere else — models can handle more than users ask of them, but trust hasn't formed yet.

Safety-wise: only 0.8% of actions are irreversible. 80% of tool calls come from agents with at least one safeguard. 73% have a human in the loop. The risk profile is much more conservative than the discourse suggests.
